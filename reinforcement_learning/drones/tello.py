...
class Tello(Drone):
	...
	def connect(self):
		...
		x = input()
		'''
		ini = _utils.prompt('send any key when connected to Tello via WiFi...')
	...
	def move(self, x, y, z, speed):
		...
		if speed < 10 or speed > 100:
			_utils.error('speed out of range for tello')
		#if min(abs(x), abs(y), abs(z)) < 100 or max(abs(x), abs(y), abs(z)) > 500:
		#    _utils.error('position out of range for tello')
		...
		while abs(x) > 0 or abs(y) > 0 or abs(z) > 0:
			...
			ini = _utils.prompt(f'go {x2} {y2} {z2} {speed}?')
			if ini == 'y':
