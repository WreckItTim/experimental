...
class Random(Spawner):
	...
	def __init__(self,
			...
			map_component, # used to detect highest collidable surface, get_roof() and inside_object() needs to be defined
			bounds_component, # valid range to spawn in 
			start_goal_range, # range of distances [meter] to create goal in range [min, max]
			levelup_goal_range = [5, 5], # will increase min,max goal distance by these many meters
			goal_range=None,
		...
		if goal_range is None:
			self.goal_range = start_goal_range.copy()
	...
	def set_level(self, level):
		self.goal_range = (np.array(self.start_goal_range) + level*np.array(self.levelup_goal_range)).tolist()
	...
	def random_location(self):
		if self.vertical:
			...
			while (True):
				...
				in_object = self._map.in_object(x, y, z)
				if not in_object:
					break
		return x, y, z
	...
	def random_goal(self, state=None):
		...
		while(True):
			...
			attempt += 1
			if attempt > 1000:
				gm.speak(f'could not find a valid goal at drone:{drone_position} and range:{self.goal_range} respawning...')
				valid_point = False
				break
		if valid_point:
			self._goal_x = goal_x
