...
class Resize(object):
    ...
    def __call__(self, img, depth):
        #assert img.size == mask.size
        assert img.size == depth.size
        return img.resize(self.size, Image.BILINEAR), depth.resize(self.size, Image.BILINEAR)
...
class RandomCrop(object):
    ...
    def __call__(self, img, depth):
        ...
        return img.crop((x1, y1, x1 + self.size, y1 + self.size)), depth.crop((x1, y1, x1 + self.size, y1 + self.size))
