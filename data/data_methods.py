...
import contexts.airsim as c_as # map_params dictionary
map_params = c_as.map_params
...
def plot_map(fig, ax, airsim_map, show_z=False, 
...
def view_map(fig, ax, x, y, z, yaw, airsim_map, 
    ...
    plot_map(fig, ax, airsim_map, show_z, 
...
def animate(airsim_map, sensor_names, ncols, nrows, frames, states, 
    ...
    def update(t):
        ...
        ax = axs[0, 0]
        ax.clear()
        view_map(fig, ax, x, y, z, yaw, airsim_map, start, target, path, False, resolution)
        ...
        for sensor_idx, observation in enumerate(observations):
            ...
            #if observation is None:
            #    print('animate null', sensor_name, sensor_idx)
            if observation is None:
...
def get_data_point(_x, _y, _z, _yaw, airsim_map, sensor_name, id_names, ):
    data_dicts = Globals.get('data_dicts')
    cache = Globals.get('cache')
...
def _data_point(x, y, z, yaw, airsim_map, sensor_names, id_names,
    ...
    for sensor_idx, sensor_name in enumerate(sensor_names):
        observation = get_data_point(x, y, z, yaw, airsim_map, sensor_name, id_names, )
        if make_animation:
...
def _data_end(airsim_map, sensor_names, make_animation, ncols, nrows, frames, states, start, target, show_path, 
              return_data, data_sets, text_blocks=None, resolution=None, sensor_psuedonames={}):
    animation = None
    if make_animation:
        animation = animate(airsim_map, sensor_names, ncols, nrows, frames, states, start, target, show_path, text_blocks, resolution, sensor_psuedonames)
    if return_data:
...
def data_at_coordinates(airsim_map, sensor_names, id_names, x_vals, y_vals, z_vals, yaw_vals, 
        ...
        for yi, y in enumerate(y_vals):
            for zi, z in enumerate(z_vals):
                for yawi, yaw in enumerate(yaw_vals):
                    _data_point(x, y, z, yaw, airsim_map, sensor_names, id_names,
                                make_animation, frames, states, ncols, nrows,
                                return_data, data_sets, include_nulls)
    animation = _data_end(airsim_map, sensor_names, make_animation, ncols, nrows, frames, states, 
...
def data_at_path(airsim_map, sensor_names, id_names, path, 
                 make_animation=True, return_data=True, ncols=2, additional_subplots=0, resolution=None, 
                 sensor_psuedonames={}, include_nulls=False):
    frames, states, data_sets, nrows = _data_start(airsim_map, sensor_names, return_data, ncols, additional_subplots)
...
def yaw_to_idx(yaw):
	...
	return yaw_idx
def data_at_episode(airsim_map, sensor_names, id_names, episode, actions, 
...
# airsim_map = 'AirSimNH' 'Blocks'
def get_data(region, sensor_names, airsim_map, id_names=['alpha']):
    assert region in ['train', 'test', 'all'], 'invalid region use either train test or all'
    # set coordinate ranges
    x_bounds, y_bounds, z_bounds = c_as.get_bounds(airsim_map, region)
    ...
    location_data, location_animation = data_at_coordinates(airsim_map, sensor_names, id_names, x_vals, y_vals, z_vals, yaw_vals, 
                        make_animation=False, return_data=True)
