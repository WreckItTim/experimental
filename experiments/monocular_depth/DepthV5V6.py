...
scheduler_name = 'th.optim.lr_scheduler.StepLR'
scheduler_params = str({'step_size':30}).replace(' ', '')
optimizer_name = 'th.optim.SGD'
all_jobs = []
for version in ['V5', 'V6']:
    for scale in version_dict:
        for random_seed in range(4):
            version_name = f'{version}/{version_dict[scale]}'
            run_dir = f'models/monocular_depth/{version_name}/seed_{random_seed}/'
            if os.path.exists(f'{run_dir}r2s.p'):
               continue
            if version == 'V5':
                loss_name = 'th.nn.MSELoss'
            if version == 'V6':
                loss_name = 'th.nn.L1Loss'
